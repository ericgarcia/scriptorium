...
def index_losses(live, bundle):
    """Entries the live index lists that the bundle's index.json would drop.
    ...
    """
    key = lambda p: (p.get('slug'), p.get('kind', 'piece'))
    have = {key(p) for p in (bundle or {}).get('pieces', [])}
    have_slugs = {s for s, _k in have}
    lost = {key(p) for p in (live or {}).get('pieces', [])} - have
    # A slug that disappears entirely is named plainly. One that survives only as ANOTHER
    # KIND is named with its kind: a talk and its companion essay share a slug, so a bundle
    # that keeps the essay and drops the talk still carries the slug, and a slug-only check
    # would watch the talk go without a word. That is the drop this guard most needs to see.
    return sorted(s if s not in have_slugs else f'{s} ({k})' for s, k in lost)
